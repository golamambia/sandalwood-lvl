...
class PageController extends Controller
{
	...
	public function insert(Request $request)
	{
		...
		$page_template = $request->page_template>0?$request->page_template:0;
				...
				return Redirect::to('admin/page/edit/'.$page_id)->with('admin_msg', 'Page has been added successfully.');
	...
	public function service()
	{
		...
		$column_array = array('id' => 'Id', 'page_name' => 'Service Name', 'parent_id' => 'Parent', 'menu_order' => 'Order');
		$search = Request()->search;
	...
	{
		$all_pages = Page::get();//where('posttype','service')->
		return view('admin.service.add', compact('all_pages'));
	}

	/* Admin insert service Post*/
	public function serviceinsert(Request $request)
	{
		...
		$page_template = $request->page_template>0?$request->page_template:0;
				...
				//return redirect()->back()->with('success', true);
				return Redirect::to('admin/service/edit/'.$page_id)->with('admin_msg', 'Service has been added successfully.');
	...
	/* Admin Update service Get*/
	public function serviceedit($id)
	{
		$all_pages = Page::where('id','!=',$id)->get();//where('posttype','service')->
		$page = Page::where('id',$id)->where('posttype','service')->get();
		$page_extra = PageExtra::where('page_id',$id)->orderBy('type', 'asc')->orderBy('rank', 'asc')->get();

		return view('admin.service.edit', compact('page','page_extra','all_pages'));
	}

	/* Admin Update service Post*/
